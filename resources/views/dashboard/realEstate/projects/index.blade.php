            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body table-responsive">
                        ...
                        <form method="GET">
                            @php
                            $seletectCompletionStatus =request()->completion_status_ids?request()->completion_status_ids:[];
                            $seletectAccommodations =request()->accommodation_id?request()->accommodation_id:[];
                            ...
                            <div class="row mb-2">
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        ...
                                        <select multiple="multiple" data-placeholder="Select Communities" style="width: 100%;" class=" form-control select1" id="community_ids" name="community_ids[]">
                                            @foreach ($communities as $key=>$value)
                                            <option value="{{ $key }}" @if (in_array($key, $seletectCommunities)) selected @endif>{{ $value }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        ...
                                        <select multiple="multiple" data-placeholder="Select Developers" style="width: 100%;" class=" form-control select1" id="developer_ids" name="developer_ids[]">
                                            @foreach ($developers as $key=>$value)
                                            <option value="{{ $key }}" @if (in_array($key, $seletectDevelopers)) selected @endif>{{ $value }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="col-sm-2">
                                    <div class="form-group">
                                        ...
                                        <select multiple="multiple" data-placeholder="Select Completion Status" style="width: 100%;" class=" form-control select1" id="completion_status_id" name="completion_status_ids[]">
                                            @foreach ($completionStatuses as $key=>$value)
                                            <option value="{{ $key }}" @if (in_array($key, $seletectCompletionStatus)) selected @endif>{{ $value }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="col-sm-2">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="display_on_home" name="display_on_home">
                                            ...
                                            <option value="1" @if(request()->display_on_home === '1') selected @endif>Yes</option>
                                            <option value="0" @if(request()->display_on_home === '0') selected @endif>No</option>
                                ...
                                <div class="col-sm-4">
                                    ...
                                    <input type="text" value="{{ request()->keyword }}" class="form-control" id="keyword" placeholder="Enter Name" name="keyword">
                                </div>
                            </div>
                            <br>
                            <div class="row">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <a class="btn btn-block btn-warning search_clear_btn" href="{{ url('dashboard/projects') }}">Clear Search</a>
                                    @endif
                                </div>
                        ...
                        <table class="table table-hover text-nowrap table-striped propertyDatatable">
                            <thead>
                                <tr>
                                    ...
                                    <th>Status</th>
                                    <th>Order Number <span class="arrow up" onclick="orderBy('projectOrder', 'asc')">&#x25B2;</span><span class="arrow down" onclick="orderBy('projectOrder', 'desc')">&#x25BC;</span></th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $project->completionStatus ? $project->completionStatus->name: '' }}</td>
                                    <td>
                                        <span class="badge @if ($project->is_display_home === 1) bg-success @else bg-danger @endif">
                                            @if ($project->is_display_home === 1) Yes @else No @endif
                                        </span>
                                    </td>
                                    <td>
                                    ...
                                    <td class="project-actions text-right">
                                        <form method="POST" action="{{ route('dashboard.projects.destroy', $project->id) }}">
                                            @csrf
                                            @method('DELETE')
                                            <a class="btn btn-warning btn-sm" href="{{ config('app.frontend_url')."projects/".$project->slug }}" target="_blanket">
                                            ...
                                            @endif
                                            @if($project->brochure)
                                            <a class="btn btn-dark btn-sm" href="{{ $project->brochure }}" target="_blanket">
                                                <i class="fas fa-file"></i>
                                                View Brochure
                                            </a>
                                            @endif
                                            <a class="btn btn-success btn-sm" href="{{ route('dashboard.projects.paymentPlans', $project->id) }}">
