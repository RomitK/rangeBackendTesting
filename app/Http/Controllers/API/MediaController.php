...
class MediaController extends Controller
{
    public function index(Request $request)
    {
        try{
            ...
            [
                'all'=> MediaListResource::collection($medias->paginate(40))->response()->getData(true),
                'news'=>MediaListResource::collection($news->where('article_type','News')->paginate(40))->response()->getData(true),
                'blogs'=>MediaListResource::collection($blogs->where('article_type','Blogs')->paginate(40))->response()->getData(true),
                'awards'=>MediaListResource::collection($awards->where('article_type','Awards')->paginate(40))->response()->getData(true),
                'celebrations'=>MediaListResource::collection($celebrations->where('article_type','Celebrations')->paginate(40))->response()->getData(true),
                'events'=>MediaListResource::collection($events->where('article_type','Sales Event')->paginate(40))->response()->getData(true),
            ]
            , 200);
